...
def make_requests(steps_data: dict, operation_schemas: dict, fail_fast: bool, verbose: bool):
    ...
    for s in steps_data:
        ...
        try:
            response_schema = operation_schemas[s['operation_id']]['responses'][str(s['status_code'])]
        except KeyError as e:
            print("[bold red]Response Validation Error[/bold red]")
            raise KeyError(f"Response code of {e} not found in responses for {s['operation_id']}") from e
        ...
        if 'type' in operation_schemas[s['operation_id']]['responses'][str(s['status_code'])].keys():
            response_type = operation_schemas[s['operation_id']]['responses'][str(s['status_code'])]['type']
